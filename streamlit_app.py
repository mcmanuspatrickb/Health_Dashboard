...
def load_data(source: str, uploaded_file=None, page_size: int = 10, max_pages: int = 10):
    if source == "Hevy API":
        api_key = get_secret("HEVY_API_KEY")
        if not api_key:
            st.warning("No HEVY_API_KEY found in Streamlit secrets. Falling back to CSV/upload mode.")
            raw = load_from_csv(uploaded_file)
            return normalize_hevy_table(raw), "CSV fallback"

        try:
            workouts = fetch_hevy_workouts(api_key, page_size=page_size, max_pages=max_pages)
            raw = flatten_hevy_workouts(workouts)
            return normalize_hevy_table(raw), "Hevy API"
        except Exception as e:
            st.error(f"Hevy API failed: {e}")
            st.warning("Falling back to CSV/upload mode.")
            raw = load_from_csv(uploaded_file)
            return normalize_hevy_table(raw), "CSV fallback"

    raw = load_from_csv(uploaded_file)
    return normalize_hevy_table(raw), "CSV/upload"
...
def prepare_body_calculations(
    ...
    baseline_muscle = float(
        proxy_config.get("muscle_mass_baseline_kg", 74.28)
    )
    ...
    paired["muscle_retention_pct"] = (
        paired["estimated_muscle_mass_kg"] / baseline_muscle * 100
    )
...
with st.sidebar:
    ...
    default_source = "Hevy API" if get_secret("HEVY_API_KEY") else "CSV/upload"
    source = st.radio(
        "Hevy source",
        options=["Hevy API", "CSV/upload"],
        index=0 if default_source == "Hevy API" else 1,
    )
    ...
    uploaded = st.file_uploader("Optional Hevy CSV fallback", type=["csv"])
...
with overview_tab:
    ...
    primary_goals = goal_progress[
        goal_progress["goal"].isin(
            [
                "Body weight",
                "Body fat",
                "Back squat — estimated 1RM/BW",
                "Bench press — estimated 1RM/BW",
                "Deadlift — estimated 1RM/BW",
                "Pull-ups",
            ]
        )
    ]
    if not primary_goals.empty:
        cards = st.columns(3)
        for index, (_, goal) in enumerate(primary_goals.iterrows()):
            with cards[index % 3]:
                st.metric(
                    goal["goal"],
                ...
                if pd.notna(goal.get("progress_pct")):
                    st.progress(int(max(0, min(100, goal["progress_pct"]))))
...
with body_tab:
    ...
    with body_left:
        ...
        muscle_baseline = float(body_proxy["muscle_mass_baseline_kg"])
        retention_target = float(body_proxy["muscle_retention_target_pct"])
        b = st.columns(2)
        ...
        b3[1].metric(
            "Muscle retention vs baseline",
            f"{latest_paired['muscle_retention_pct_7d_median']:.1f}%"
            if latest_paired is not None
            else "—",
            f"{latest_paired['muscle_retention_pct_7d_median'] - retention_target:+.1f} pp vs {retention_target:.0f}%"
            if latest_paired is not None
            else None,
        )
        ...
        if not body_calc.empty:
            ...
            st.caption(
                ...
                f"Retention is compared with the {muscle_baseline:.2f} kg "
                "direct-Withings baseline. Trends use a 7-day rolling median. "
