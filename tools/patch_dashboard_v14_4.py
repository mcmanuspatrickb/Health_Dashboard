...
def one(old: str, new: str) -> None:
    ...
    n = s.count(old)
    if n != 1:
        raise RuntimeError(f"Expected 1 match, found {n}: {old[:100]!r}")
...
one('''        st.subheader("Latest 30 days — activity")
''', '''        st.subheader(f"Latest {trend_days} days — activity")
''')
one('''                health_steps.tail(30),
''', '''                trend_window(health_steps, "date", trend_days),
''')
one('''        st.subheader("Latest 30 days — training load")
...
one('''].sort_values("start_time").tail(20)
...
one('''        n[3].metric("Logged days — last 30", len(recent))
''', '''        n[3].metric(f"Logged days — last {trend_days}", len(recent))
''')

one('''            recent = recovery.tail(60).copy()
...
one('''            stages = main_sleep.tail(30)[
''', '''            stages = main_sleep[
''')
one('''        bp_recent = withings_bp.sort_values("date").tail(60).copy()
