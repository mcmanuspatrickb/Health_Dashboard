...
import_anchor = '''from renpho_data import (
    delete_renpho_measurement,
    insert_renpho_measurement,
    load_renpho_measurements,
)
'''
import_replacement = import_anchor + '''from recomposition import build_recomposition_summary
'''
if "from recomposition import build_recomposition_summary" not in text:
    if import_anchor not in text:
        raise SystemExit("Import anchor not found")
    text = text.replace(import_anchor, import_replacement, 1)

section_anchor = '''    "Strength Progress",
    "Endurance",
'''
section_replacement = '''    "Strength Progress",
    "Fat Loss & Muscle Preservation",
    "Endurance",
'''
if '"Fat Loss & Muscle Preservation"' not in text:
    if section_anchor not in text:
        raise SystemExit("Section-list anchor not found")
    text = text.replace(section_anchor, section_replacement, 1)

load_anchor = '''elif selected_section == "Endurance":
    endurance = safe_frame(
'''
load_block = '''elif selected_section == "Fat Loss & Muscle Preservation":
...
'''
if 'elif selected_section == "Fat Loss & Muscle Preservation":\n    health_steps' not in text:
    if load_anchor not in text:
        raise SystemExit("Data-loading anchor not found")
    text = text.replace(load_anchor, load_block + load_anchor, 1)

render_anchor = '''elif selected_section == "Body Composition & Nutrition":
    st.header("Body composition and nutrition")
'''
render_block = r'''elif selected_section == "Fat Loss & Muscle Preservation":
...
'''
if 'st.header("Fat Loss & Muscle Preservation")' not in text:
