...
DEFAULT_ALIAS_RULES = [
    ...
    },
    {
        ...
        "notes": "Standard push-ups; one-minute verification requires set duration data.",
    },
    {
        "goal_id": "bar_hang",
    ...
    {
        "goal_id": "row",
        "pattern": r"(?i)\b(barbell\s*row|bent[ -]?over\s*row|pendlay\s*row|row\s*\(barbell\))\b",
        "exclude_pattern": r"(?i)\b(dumbbell|cable|machine|chest.supported|t.bar)\b",
        "notes": "Barbell row variations only.",
    },
    {
...
def load_body_composition_proxy(path: str | Path) -> dict[str, float | str | list]:
    ...
    return {
        ...
        "muscle_mass_baseline_kg": float(proxy.get("muscle_mass_baseline_kg", 74.28)),
        "muscle_retention_target_pct": float(proxy.get("muscle_retention_target_pct", 97.0)),
...
def _body_snapshot(
    ...
    baseline_muscle = float(proxy_config.get("muscle_mass_baseline_kg", 74.28))
    ...
    retention_pct = (
        estimated_muscle_mass / baseline_muscle * 100
        if estimated_muscle_mass is not None and baseline_muscle > 0
        else None
    )
    return {
        ...
        "muscle_retention_pct": retention_pct,
    }
...
def build_goal_progress(
    ...
    for goal in goals:
        ...
        elif metric == "muscle_retention_pct":
            current = body_values["muscle_retention_pct"]
            muscle_kg = body_values["estimated_muscle_mass_kg"]
            baseline = float(proxy_config.get("muscle_mass_baseline_kg", 74.28))
            current_text = (
                f"{current:.1f}% ({muscle_kg:.2f} kg vs {baseline:.2f} kg)"
                if current is not None and muscle_kg is not None
                else "—"
            )
